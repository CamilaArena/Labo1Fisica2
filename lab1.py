...
# Función para calcular el potencial eléctrico 
...
# Obtener el punto de referencia donde se calcularán el campo y el potencial eléctrico
...
def graficar(Ex, Ey, V, cargas, x0, y0):
    ...
    for q, pos in cargas:
        if q > 0:
            if not pos_legend:  # Solo añadir la leyenda una vez
                plt.scatter(pos[0], pos[1], color='red', edgecolor='black', s=150, marker='o', zorder=5, label='Carga positiva')
                pos_legend = True
            else:
                plt.scatter(pos[0], pos[1], color='red', edgecolor='black', s=150, marker='o', zorder=5)
        else:
            if not neg_legend:
                plt.scatter(pos[0], pos[1], color='blue', edgecolor='black', s=150, marker='o', zorder=5, label='Carga negativa')
                neg_legend = True
            else:
                plt.scatter(pos[0], pos[1], color='blue', edgecolor='black', s=150, marker='o', zorder=5)
    ...
    # Valores calculados para que la graduación de colores del potencial se muestre correctamente
    vmin, vmax = np.min(V), np.max(V)
    linthresh = (vmax - vmin) / 100  

    # Graficar el potencial eléctrico usando un mapa de contornos
    plt.figure(figsize=(8, 6))
    contour = plt.contourf(X, Y, V, levels=150, cmap='plasma',
                           norm=SymLogNorm(linthresh=linthresh, linscale=0.5, vmin=vmin, vmax=vmax))
    plt.colorbar(contour)

    # Graficar las cargas con diferentes colores
    for q, pos in cargas:
        if q > 0:
            plt.scatter(pos[0], pos[1], color='red', edgecolor='white', s=150, marker='o', zorder=5)
        else:
            plt.scatter(pos[0], pos[1], color='blue', edgecolor='white', s=150, marker='o', zorder=5)
    
    plt.scatter(x0, y0, color='white', label='Punto de referencia', zorder=6)
    plt.xlabel('x')
    plt.ylabel('y')
